...
# Store some statistics about when we last queried the endpoint, and holds a mutex.
api_info = {}

# Initialize parameters for all our API lists.
for i in query_normal_api_list + aa_api_list + high_limit_list:
...
async def try_api_request(
    request: str, endpoints=query_normal_api_list, post_body=None, session=None
) -> dict:
    ...
    while True:  # Until success
        for _ in range(0, len(endpoints)):  # Iterate through all endpoints
            ...
            if api_info[current_api]["ratelimit"] > 5:
                ratelimit_current = (
                    60 * 2 ** api_info[current_api]["ratelimit"]
            ...
            if (
                time.time() - api_info[current_api]["last_failed"] < ratelimit_current
                or time.time() - api_info[current_api]["last_tried"] < 1
            ):
            ...
            try:
                ...
                print(
                    "Request returned: ",
                    data["code"],
                    " is this a reason to fail/retry the request?",
                )
            except:
                pass
            try:
            ...
            if (
                not "actions" in data.keys()
                and not "data" in data.keys()
                and (not "get_creator" in request and not "creator" in data.keys())
                and (
                    not "get_created_accounts" in request
                    and not "accounts" in data.keys()
                )
                and (request != "/v1/chain/get_info")
                and post_body == None
                or post_body != None
                and not "transactions" in data.keys()
            ):
            ...
            api_info[current_api][
                "ratelimit"
            ] = 1  # Reset ratelimit upon a succesfull request
        ...
        if backoff_factor > 50:  # Shouldn't happen unless something is wrong.
            print(
                "We tried all API endpoints and all are rate limiting us. Are you sure this is a valid request?",
                backoff_factor,
                request,
            )
        backoff_factor += 1
...
try:
    ...
    async def drop_random_from_wallet(
        bot,
        drop_message,
        member,
        memo: str,
        ...
        available_assets = (
            await try_api_request(
                f"/atomicassets/v1/assets?owner={sender}&page=1&limit=1000",
                endpoints=aa_api_list,
            )
        )["data"]
        if len(available_assets) == 0:
            await drop_message.channel.send(
                f"Error sending NFT: No assets in wallet {sender} found."
            )
            return
        ...
        try:
            if str(member.id) in bot.linked_wallets:
                # User has a linked wallet -> directly send the NFT
                await directly_send_nft(
                    bot=bot,
                    drop_message=drop_message,
                    member=member,
                    assets=[to_send],
                    dropped_asset=choosen_asset,
                    memo=memo,
                    log_additional=log_additional,
                    emoji_sequences=emoji_sequences,
                    account=account,
                    sender=sender,
                    additional_auths=additional_auths,
                )
                return

        except Exception as e:
            await drop_message.channel.send(
                f"Ran into an error directly sending the NFT to linked wallet: {e}\n"
            )
            return
        await send_claimlink_nft(
            bot=bot,
            drop_message=drop_message,
            member=member,
            assets=[to_send],
    ...
    async def directly_send_nft(
        ...
        log_message = (
            f"User {member.name} received asset <https://wax.atomichub.io/explorer/asset/wax-mainnet/{' '.join(assets)}> directly to their wallet <https://wax.bloks.io/account/{target}>."
            + log_additional
        )[0:969]
    ...
    async def send_claimlink_nft(
        ...
        claimlink = await gen_claimlink(
            assets,
            memo=memo + settings.DROP_EXTRA_INFO,
            account=account,
            sender=sender,
            additional_auths=additional_auths,
        )
        ...
        if "https://wax.atomichub.io/trading/link/" not in claimlink:
            await drop_message.channel.send(
                f"Link generation failed! {claimlink}. Please try again and/or ping {settings.MAINTAINER}"
            )
            return
        ...
        channel = bot.get_channel(settings.LOG_CHANNEL)
        log_message = (
            f"User {member.name} received claimlink <{claimlink.split('?key')[0]}>"
            + log_additional
        )[0:969]
        await channel.send(log_message)
        await drop_message.add_reaction(emoji_sequences[1])
    ...
    async def send_asset(
        asset_ids: List[int],
        receiver: str,
        account: EosAccount = account,
        memo="",
        sender="",
        additional_auths=[],
    ):
        authorization = [
            account.authorization(settings.WAX_PERMISSION)
        ] + additional_auths
        memo = str(memo)[:256]
        if sender == "":
            sender = account.name
    ...
    async def gen_claimlink(
        ...
        if resp == False:
            raise Exception(
                f"Failed to create claimlink, please try again. Error: {msg}"
            )
        tx_id = msg["transaction_id"]
        try:
            newresp = await try_api_request(
                f"/v2/history/get_transaction?id={tx_id}", query_normal_api_list
            )
    ...
    async def doAction(
        action_array, api_rpc, account, ref_block={}, index=None, retry=5
    ):
        """Very basic helper function to create a transaction"""
        if index is None:
            index = (secrets.randbelow(len(api_rpc)) + random.randint(0, 5)) % len(
                api_rpc
            )
        for i in range(0, retry):
            try:
                ...
                resp = await rpc.sign_and_push_transaction(
                    transaction, keys=[account.key]
                )
                return True, resp
            except Exception as e:
                try:
        ...
        while len(asset_ids) > 0:
            action = EosAction(
                ...
                data={
                    "from": from_addr,
                    "to": to_addr,
                    "asset_ids": asset_ids[0 : min(423, len(asset_ids))],
                    "memo": memo,
                },
            )
