...
class Drop(commands.Cog):
    ...
    async def drop(self, ctx, member: discord.Member, *, memo: str = settings.DEFAULT_DROP_MEMO):
        try:
            ...
            await utils.drop_random_from_wallet(bot = self.bot, drop_message = ctx.message, member = member, memo =  memo, log_additional =  log_additional, emoji_sequences = settings.react_emoji_sequence[1:])
    ...
    async def stab(self, ctx, member: discord.Member, *, memo: str = settings.DEFAULT_DROP_MEMO):
        ...
        try:
            ...
            await utils.drop_random_from_wallet(bot = self.bot, drop_message = ctx.message, member = member, memo =  memo, log_additional =  log_additional, emoji_sequences = settings.react_emoji_sequence[1:], sender = special_wallet, additional_auths= special_auth)
