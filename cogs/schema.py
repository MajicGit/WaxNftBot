...
async def fetch_schemas(collection_name=settings.COLLECTION_NAME):
    r = await utils.try_api_request(
        f"/atomicassets/v1/schemas?collection_name={collection_name}",
        endpoints=utils.aa_api_list,
    )
    return r["data"]
...
async def query_templates(schema_name, collection_name=settings.COLLECTION_NAME):
    r = await utils.try_api_request(
        f"/atomicassets/v1/templates?collection_name={collection_name}&schema_name={schema_name}&has_assets=true",
        endpoints=utils.aa_api_list,
    )
    return r["data"]
...
async def gen_schema():
    ...
    for i in await fetch_schemas():
        ...
        for j in await query_templates(name, settings.COLLECTION_NAME):
            ...
            try:
                ...
                try:
                    card_name = (
                        await utils.try_api_request(
                            f"/atomicmarket/v1/assets?collection_name={settings.COLLECTION_NAME}&template_id={template_id}&page=1&limit=11&order=desc&sort=asset_id",
                            endpoints=utils.aa_api_list,
                        )
                    )["data"][0]["data"]["name"].split("-#")[0]
    ...
    for i in settings.SCHEMAS_GROUPED:
        ...
        schema_data = {"name": display_name, "filters": filter_list}
        schemas.append(schema_data)

    schemas_alone = list(settings.SCHEMA_NAME_MAP.items()) + [
        [i, i]
        for i in all_schemas
        if i not in settings.SCHEMA_NAME_MAP and i not in settings.SCHEMA_IGNORE
    ]
    ...
    final = {
        "name": settings.DISPLAY_COLLECTION_NAME,
        "data": templates,
        "series": schemas,
    }
...
class Schema(commands.Cog):
    """
    Cog for automatic generation of atomichub schemas.
    Needs a lot of customization to get working nicely :) gl
    Hope example settings are helpful.
    """
    ...
    @commands.command(description="Get AH Collection Book", aliases=["collectionbook"])
    ...
    async def schema(self, ctx):
        try:
            ...
            await ctx.send(
                ...
                file=discord.File(f"{settings.COLLECTION_NAME}_schema.json"),
            )
            await ctx.message.add_reaction("✔️")
        except Exception as e:
            await ctx.send(f"Ran into an error {e} please ping {settings.MAINTAINER}\n")
